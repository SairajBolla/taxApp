...
public interface IncomeTaxService {
	...
	default Double getTaxPercent(final double grossIncome, final int age) {
		...
		if (category == Category.GENERAL) {
			...
			return netIncome <= generalSalarySlabs[0] ? calculateTax(netIncome, 0.0, generalSalarySlabs)
					: netIncome <= generalSalarySlabs[1] ? calculateTax(netIncome, 5.0, generalSalarySlabs)
							: netIncome <= generalSalarySlabs[2] ? calculateTax(netIncome, 20.0, generalSalarySlabs)
									: calculateTax(netIncome, 20.0, generalSalarySlabs);

		} else {

			return netIncome <= seniorCitizenSalarySlabs[0] ? calculateTax(netIncome, 0.0, seniorCitizenSalarySlabs)
					: netIncome <= seniorCitizenSalarySlabs[1] ? calculateTax(netIncome, 5.0, seniorCitizenSalarySlabs)
							: netIncome <= seniorCitizenSalarySlabs[2]
									? calculateTax(netIncome, 20.0, seniorCitizenSalarySlabs)
									: calculateTax(netIncome, 20.0, seniorCitizenSalarySlabs);
		}
	}
	...
	default double calculateTax(final double netIncome, final double taxPercent, final Double taxSlabs[]) {
		...
		if (taxPercent == 5.0) {
			...
			totalTax = Math.abs(((Math.abs(netIncome - taxSlabs[0]) * 5.0)
					+ (Math.abs(netIncome - taxSlabs[0] - taxSlabs[1]) * 20.0)) / 100);
		}
		return totalTax;
	}
}
